...
const TRIGGER_LABELS: Record<string, string> = {
  ...
  day_before: 'Upcoming Reminder',
  follow_up: 'Follow-Up',
  six_month_recall: '6-Month Recall',
...
const TRIGGER_OPTIONS = [
  ...
  { value: 'day_before', label: 'Upcoming Reminder' },
  { value: 'follow_up', label: 'Follow-Up' },
  { value: 'six_month_recall', label: '6-Month Recall' },
...
export default function NotificationsClient({
  ...
  const refreshLogs = async (clinic = clinicFilter, trigger = triggerFilter) => {
    const result = await fetchNotifications(
      clinic === 'all' ? undefined : Number(clinic),
      undefined,
      trigger === 'all' ? undefined : trigger
    )
    if (result.success) {
  ...
  const handleClinicChange = (clinic: string) => {
    ...
    refreshLogs(clinic, triggerFilter)
  }

  const handleTriggerChange = (trigger: string) => {
    setTriggerFilter(trigger)
    refreshLogs(clinicFilter, trigger)
  }
            ...
            <tbody className="divide-y divide-gray-100 text-sm text-slate-705">
              {filteredNotifications.map((notif) => (
                <tr key={notif.id} className="hover:bg-gray-50/50 transition">
                  <td className="px-6 py-4">
                    <div>
                      <p className="font-semibold text-slate-900">
                        {notif.patients ? `${notif.patients.first_name} ${notif.patients.last_name}` : '—'}
                      </p>
                      <p className="text-xs text-gray-500">{formatPhone(notif.patients?.phone)}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-xs text-slate-700">
                    {notif.appointments?.clinics?.name ?? '—'}
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-xs text-slate-700">
                      {TRIGGER_LABELS[notif.trigger_type] ?? notif.trigger_type}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${getStatusBadgeClass(notif.status ?? '')}`}>
                        {notif.status}
                      </span>
